...
```{r, include = FALSE}
knitr::opts_knit$set(root.dir = "E:\\Uni\\Forschung\\Article\\2019 - MASS",
              fig.pos = "H")
...
```{r, eval = FALSE}
# mkdirs(paste0("=Final Data OUT=/Plots_SM_G_Kor/Plots_SM_G_Kor_", PageLabel))

# if(!is.na(SM_Value[i,4]) & SM_Value[i,4]>0  ){
#      png(filename=paste0("./=Final Data OUT=/Plots_SM_G_Kor/Plots_SM_G_Kor_", PageLabel, "/Plot_", PageLabel, "_lfdn_", i,".png"),
#          width     = 2400,
#          height    = 2400,
#          units     = "px",
#          res       = 1200,
...
#      plot(SM_Value2_Kor, type = "l", lwd=0.2, col = "red", ylim=c(0,5),main = paste0("SM_",PageLabel,"_lfdn_",i ))
#      abline(h=mean(SM_Value2_Kor), col = "blue", lwd=0.4)
#      abline(h=sd(SM_Value2_Kor), col = "green", lwd=0.4)
#      dev.off()
#    }
```

```{r echo = FALSE, out.width = '60%', fig.align = "center", fig.cap = "Examples of total acceleration profiles (n = 5)"}
...
p1 <- sm %>%
  ...
  filter(var < 2000) %>%
  ggplot() + 
  geom_line(aes(x = var, y = value, group = rowname), alpha = 0.2) + 
  geom_smooth(aes(x = var, y = value), se = FALSE, color = "red") +
  ylim(0, 5) +
